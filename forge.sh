...
echo "fragrant-firefly" > /etc/hostname
sed -i 's/127\.0\.0\.1.*localhost/127.0.0.1	fragrant-firefly.localdomain fragrant-firefly localhost/' /etc/hosts
hostname fragrant-firefly
...
PASSWORD=$(mkpasswd bdBseXZQOAay5tgaE6wq)
...
# Laravel Forge
ssh-rsa  worker@forge.laravel.com
...
debconf-set-selections <<< "mysql-community-server mysql-community-server/root-pass password hvLK3RxbZPoduyNQLZR0"
debconf-set-selections <<< "mysql-community-server mysql-community-server/re-root-pass password hvLK3RxbZPoduyNQLZR0"
...
mysql --user="root" --password="hvLK3RxbZPoduyNQLZR0" -e "GRANT ALL ON *.* TO root@'18.216.80.188' IDENTIFIED BY 'hvLK3RxbZPoduyNQLZR0';"
mysql --user="root" --password="hvLK3RxbZPoduyNQLZR0" -e "GRANT ALL ON *.* TO root@'%' IDENTIFIED BY 'hvLK3RxbZPoduyNQLZR0';"
service mysql restart

mysql --user="root" --password="hvLK3RxbZPoduyNQLZR0" -e "CREATE USER 'forge'@'18.216.80.188' IDENTIFIED BY 'hvLK3RxbZPoduyNQLZR0';"
mysql --user="root" --password="hvLK3RxbZPoduyNQLZR0" -e "GRANT ALL ON *.* TO 'forge'@'18.216.80.188' IDENTIFIED BY 'hvLK3RxbZPoduyNQLZR0' WITH GRANT OPTION;"
mysql --user="root" --password="hvLK3RxbZPoduyNQLZR0" -e "GRANT ALL ON *.* TO 'forge'@'%' IDENTIFIED BY 'hvLK3RxbZPoduyNQLZR0' WITH GRANT OPTION;"
mysql --user="root" --password="hvLK3RxbZPoduyNQLZR0" -e "FLUSH PRIVILEGES;"

# Create The Initial Database If Specified

mysql --user="root" --password="hvLK3RxbZPoduyNQLZR0" -e "CREATE DATABASE mydb CHARACTER SET utf8 COLLATE utf8_unicode_ci;"
...
curl --insecure --data "event_id=13509727&server_id=161127&sudo_password=bdBseXZQOAay5tgaE6wq&db_password=hvLK3RxbZPoduyNQLZR0&recipe_id=" https://forge.laravel.com/provisioning/callback/app
