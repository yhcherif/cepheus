...
echo "colorful-firefly" > /etc/hostname
sed -i 's/127\.0\.0\.1.*localhost/127.0.0.1	colorful-firefly.localdomain colorful-firefly localhost/' /etc/hosts
hostname colorful-firefly
...
PASSWORD=$(mkpasswd g32AzZ7wv3FfpClz4tfD)
...
# Laravel Forge
ssh-rsa  worker@forge.laravel.com
...
debconf-set-selections <<< "mysql-community-server mysql-community-server/root-pass password KNSHr9zLLNnVyEozupAV"
debconf-set-selections <<< "mysql-community-server mysql-community-server/re-root-pass password KNSHr9zLLNnVyEozupAV"
...
mysql --user="root" --password="KNSHr9zLLNnVyEozupAV" -e "GRANT ALL ON *.* TO root@'18.216.80.188' IDENTIFIED BY 'KNSHr9zLLNnVyEozupAV';"
mysql --user="root" --password="KNSHr9zLLNnVyEozupAV" -e "GRANT ALL ON *.* TO root@'%' IDENTIFIED BY 'KNSHr9zLLNnVyEozupAV';"
service mysql restart

mysql --user="root" --password="KNSHr9zLLNnVyEozupAV" -e "CREATE USER 'forge'@'18.216.80.188' IDENTIFIED BY 'KNSHr9zLLNnVyEozupAV';"
mysql --user="root" --password="KNSHr9zLLNnVyEozupAV" -e "GRANT ALL ON *.* TO 'forge'@'18.216.80.188' IDENTIFIED BY 'KNSHr9zLLNnVyEozupAV' WITH GRANT OPTION;"
mysql --user="root" --password="KNSHr9zLLNnVyEozupAV" -e "GRANT ALL ON *.* TO 'forge'@'%' IDENTIFIED BY 'KNSHr9zLLNnVyEozupAV' WITH GRANT OPTION;"
mysql --user="root" --password="KNSHr9zLLNnVyEozupAV" -e "FLUSH PRIVILEGES;"

# Create The Initial Database If Specified

mysql --user="root" --password="KNSHr9zLLNnVyEozupAV" -e "CREATE DATABASE ecma CHARACTER SET utf8 COLLATE utf8_unicode_ci;"
...
curl --insecure --data "event_id=13509578&server_id=161117&sudo_password=g32AzZ7wv3FfpClz4tfD&db_password=KNSHr9zLLNnVyEozupAV&recipe_id=" https://forge.laravel.com/provisioning/callback/app
